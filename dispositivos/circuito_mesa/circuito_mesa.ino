...
void connect_mqttServer() {
  ...
  while (!client.connected()) {
        ...
        // Attempt to connect
        if (client.connect("cyberDeck")) { // Change the name of client here if multiple ESP32 are connected
          //attempt successful
          Serial.println("connected");
          // Subscribe to topics here
          client.subscribe("mesa");
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "mesa") {
      if(messageTemp == "lamp"){
        Serial.println("comando para a lampada");
        digitalWrite(lampada,!digitalRead(lampada));
      }
      if(messageTemp == "vento"){
        Serial.println("comando para o ventilador");
